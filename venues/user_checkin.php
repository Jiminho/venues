...
<?php 
	...
	// We are checking the check-ins using a specific user's id
	if(isset($_GET['user'])) {
		$id = $_GET['user'];					// Set id as the one that is provided by the url
		// We are not logged in
		if($uid == null || $uid == 'GUEST') {
			$is_owner = false;					// it is not our check-ins, since we are not logged in
			$is_guest = true;					// we are in guest mode, since we are not logged in
		} else {
			$is_guest = false;								// We are logged in
			if ($_GET['user'] == $uid) $is_owner = true;	// We are checking our own check-ins
			else $is_owner = false;							// We are checking another user's check-ins
		}
	} else {		// No id is given in url. We want to check our own check-ins
		$id = $uid;								// The id we are using is ours
		if($uid == null || $uid == 'GUEST') {	// But we are not logged in
			$is_guest = true;					// We are in guest mode, since we are not logged in
			$is_owner = false;					// Guests do not have their own check-ins
			header("location: ".$root);			// So, we should go back to the home page
			exit;
		} else {								// We are logged in
			$is_guest = false;					// So, we are not a guest
			$is_owner = true;					// and we are checking our own check-ins
		}
	}
	...
	if (isset($_GET['distinct'])) $distinct = true;
	else $distinct = false;
?>
<html>
<head>
				...
				<div class="row">
					<div class="col-md-5">
						<div class="tile rounded-border">
							<h2 class="tile-title rounded-border-top" style="line-height:1.0em; padding-top:7px;"><b><i class="fa <?php if($distinct) echo 'fa-globe'; else echo 'fa-street-view'; ?> fa-2x"></i><span style="font-size:13px; vertical-align:3px;">&nbsp;&nbsp;Venues<?php if($distinct) echo " visited"; else echo " checked in"; ?></span></b></h2>
...
<?php 
							...
							{
								if($distinct)
									$sql='SELECT DISTINCT v.vid, v.vname, vct.vctimg, AVG(r.rating) FROM tCheckin c, tVenue v, tVenueCategoryType vct, tRating r WHERE c.uid = '.$id.' AND v.vid = c.vid AND v.vctid = vct.vctid AND r.vid = v.vid GROUP BY 1,2,3 ORDER BY 4 DESC;';
								else
									$sql='SELECT v.vid, v.vname, vct.vctimg, AVG(r.rating), c.cdate FROM tCheckin c, tRating r, tVenue v, tVenueCategoryType vct WHERE c.uid = '.$id.' AND c.vid = v.vid AND v.vctid = vct.vctid AND r.vid = v.vid GROUP BY 1,2,3,5 ORDER BY 5 DESC,4 DESC';
