...
<?php 
	...
	// We are checking the ratings using a specific user's id
	if(isset($_GET['user'])) {
		$id = $_GET['user'];					// Set id as the one that is provided by the url
		// We are not logged in
		if($uid == null || $uid == 'GUEST') {
			$is_owner = false;					// it is not our ratings, since we are not logged in
			$is_guest = true;					// we are in guest mode, since we are not logged in
		} else {
			$is_guest = false;								// We are logged in
			if ($_GET['user'] == $uid) $is_owner = true;	// We are checking our own ratings
			else $is_owner = false;							// We are checking another user's ratings
		}
	} else {		// No id is given in url. We want to check our own ratings
		$id = $uid;								// The id we are using is ours
		if($uid == null || $uid == 'GUEST') {	// But we are not logged in
			$is_guest = true;					// We are in guest mode, since we are not logged in
			$is_owner = false;					// Guests do not have their own ratings
			header("location: ".$root);			// So, we should go back to the home page
			exit;
		} else {								// We are logged in
			$is_guest = false;					// So, we are not a guest
			$is_owner = true;					// and we are checking our own ratings
		}
	}
				...
				<div class="row">
					<div class="col-md-5">
						<div class="tile rounded-border">
							<h2 class="tile-title rounded-border-top" style="line-height:1.0em; padding-top:7px;"><b><i class="fa fa-star fa-2x"></i><span style="font-size:13px; vertical-align:3px;">&nbsp;&nbsp;Venues Rated</span></b></h2>
...
<?php 
							...
							{
								$sql='SELECT r.vid, v.vname, vct.vctimg, AVG(r.rating), MAX(r.rdate) FROM tRating r, tVenue v, tVenueCategoryType vct WHERE r.uid = '.$id.' AND r.vid = v.vid AND v.vctid = vct.vctid GROUP BY 1,2,3 ORDER BY 5 DESC,4 DESC;';
									...
									while ($row = pg_fetch_row($result)) {
										...
										<div class="media">
											...
											<div style="float:left;">
												<b>Venue</b>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$row[1].'<br />
												<b>User Rating</b>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.round($row[3], 2).'<br />
												<b>Rating Date</b>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.substr($row[4], 0, 19).'<br /><br />
								...
								else
								{
									pg_freeresult($result);
									echo "<script type='text/javascript'>
									jError('This user has not rated any venues',
										{
											onClosed:function()
											{
